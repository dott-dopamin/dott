...
function feeStatusPanel(p){
  ...
  <div class="accounting-controls"><div class="accounting-filters">${[['all','전체'],['paid','납부'],['unpaid','미납'],['exempt','면제']].map(([v,l])=>`<button class="accounting-chip ${accountingFilter===v?'active':''}" data-fee-filter="${v}">${l}</button>`).join('')}</div><div style="display:flex;gap:8px;align-items:center"><button class="btn primary" id="bulkFeePay" disabled ${closed?'style="display:none"':''}>선택 일괄 납부처리</button><input id="feeSearch" class="field accounting-search" placeholder="이름 검색" value="${esc(accountingSearch)}"></div></div>
  ...
  const syncBulk=()=>{const n=checks.filter(c=>c.checked).length;if(bulk){bulk.disabled=n===0;bulk.textContent=n?`선택 ${n}명 일괄 납부처리`:'선택 일괄 납부처리'}if(all){all.checked=checks.length>0&&n===checks.length;all.indeterminate=n>0&&n<checks.length}};
