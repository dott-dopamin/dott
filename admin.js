...
function feeStatusPanel(p){
  ...
  <div class="accounting-controls"><div class="accounting-filters">${[['all','전체'],['paid','납부'],['unpaid','미납'],['exempt','면제']].map(([v,l])=>`<button class="accounting-chip ${accountingFilter===v?'active':''}" data-fee-filter="${v}">${l}</button>`).join('')}</div><div style="display:flex;gap:8px;align-items:center"><button class="btn primary" id="bulkFeePay" disabled ${closed?'style="display:none"':''}>일괄 납부처리</button><input id="feeSearch" class="field accounting-search" placeholder="이름 검색" value="${esc(accountingSearch)}"></div></div>
