...
function showModal(title, body, onSave){const back=document.getElementById('modal');back.innerHTML=`<div class="modal"><div class="modal-head"><h3>${esc(title)}</h3><button class="icon-btn" data-close>✕</button></div><div class="modal-body">${body}</div><div class="modal-foot"><button class="btn" data-close>취소</button><button class="btn primary" id="modalSave">저장</button></div></div>`;back.classList.add('open');back.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>back.classList.remove('open'));back.onclick=e=>{if(e.target===back)back.classList.remove('open')};back.querySelector('#modalSave').onclick=async()=>{const btn=back.querySelector('#modalSave');btn.disabled=true;try{await onSave(back);back.classList.remove('open')}catch(e){alert(e.message||e)}finally{btn.disabled=false}}}
