...
window.DOTT_DB = {
  ...
  async attendance(){const {data,error}=await db.from('attendance_members').select('*').order('name');if(error){window.DOTT_ATTENDANCE_ERROR=error;console.warn('attendance_members를 불러오지 못했습니다.',error);return []}window.DOTT_ATTENDANCE_ERROR=null;return data||[]},
