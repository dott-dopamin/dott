...
function paint(){
  ...
  root.innerHTML=`<div class="catalog-table-wrap"><table class="catalog-table"><thead><tr><th>게임명</th><th>인원</th><th>시간</th><th>난이도</th><th>장르</th><th>상태 및 위치</th><th>소유주</th></tr></thead><tbody>${list.map(listRow).join('')}</tbody></table></div>`;
}
...
(async()=>{
  const cacheKey=`dott_catalog_${kind}_v46`;
  let cached=null;
